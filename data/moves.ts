...
export const Moves: {[moveid: string]: MoveData} = {
	...
	courtchange: {
		...
		onHitField(target, source) {
			const sideConditions = [
				'tastytreats', 'mist', 'lightscreen', 'reflect', 'spikes', 'safeguard', 'tailwind', 'toxicspikes', 'stealthrock', 'waterpledge', 'firepledge', 'grasspledge', 'stickyweb', 'auroraveil', 'gmaxsteelsurge', 'gmaxcannonade', 'gmaxvinelash', 'gmaxwildfire',
			];
			let success = false;
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeTarget = [
				'reflect','tastytreats', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge',
			];
			const removeAll = [
				'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','tastytreats'
			];
	...
	drillpeck: {
		...
		pp: 20,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, distance: 1, spin: 1},
		secondary: null,
		target: "any",
		type: "Flying",
		contestType: "Cool",
	...
	electricterrain: {
		...
		condition: {
			...
			onTryAddVolatile(status, target) {
				if (!(target.isGrounded()||target.ability==='dragonblessing') || target.isSemiInvulnerable() || target.hasItem('safarihelmet')) return;
				if (status.id === 'yawn') {
					this.add('-activate', target, 'move: Electric Terrain');
					return null;
				}
			...
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Electric' && (attacker.isGrounded()||attacker.ability==='dragonblessing') && !attacker.isSemiInvulnerable() && !attacker.hasItem('safarihelmet')) {
	...
	expandingforce: {
		...
		onBasePower(basePower, source) {
			if (this.field.isTerrain('psychicterrain') && (source.isGrounded()||source.ability==='dragonblessing') && !source.hasItem('safarihelmet')) {
	...
	flamewheel: {
		...
		pp: 25,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, defrost: 1, spin: 1},
		secondary: {
			chance: 10,
			status: 'brn',
		},
	...
	gmaxwindrage: {
		...
		self: {
			onHit(source) {
				let success = false;
				const removeTarget = [
					'reflect','tastytreats', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb',
				];
				const removeAll = ['spikes', 'tastytreats', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge'];
				for (const targetCondition of removeTarget) {
	...
	grassyglide: {
		...
		onModifyPriority(priority, source, target, move) {
			if (this.field.isTerrain('grassyterrain') && (source.isGrounded()||source.ability==='dragonblessing') && !source.hasItem('safarihelmet')) {
				return priority + 1;
			}
		},
		secondary: null,
	...
	grassyterrain: {
		...
		condition: {
			...
			onBasePower(basePower, attacker, defender, move) {
				...
				if (move.type === 'Grass' && (attacker.isGrounded()||attacker.ability==='dragonblessing') && !attacker.hasItem('safarihelmet')) {
	...
	guardianofalola: {
		...
		damageCallback(pokemon, target) {
			...
			if (
				target.volatiles['protect'] || target.volatiles['banefulbunker'] || target.volatiles['royalshield'] ||
	...
	hyperdrill: {
		...
		pp: 5,
		priority: 0,
		flags: {contact: 1, mirror: 1, spin: 1},
		secondary: null,
		target: "normal",
		type: "Normal",
		contestType: "Clever",
	...
	royalshield: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (this.checkMoveMakesContact(move, source, target)) {
					this.boost({atk: -1}, source, target, this.dex.getActiveMove("Royal Shield"));
				}
				return this.NOT_FAIL;
			},
			onHit(target, source, move) {
	...
	mistyexplosion: {
		...
		selfdestruct: "always",
		onBasePower(basePower, source) {
			if (this.field.isTerrain('mistyterrain') && (source.isGrounded()||source.ability==='dragonblessing') && !source.hasItem('safarihelmet')) {
	...
	mistyterrain: {
		...
		condition: {
			...
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Dragon' && (defender.isGrounded()||defender.ability==='dragonblessing') && !defender.isSemiInvulnerable() && !defender.hasItem('safarihelmet')) {
	...
	mortalspin: {
		...
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				...
				const sideConditions = ['spikes','tastytreats', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge'];
	...
	psychicterrain: {
		...
		condition: {
			...
			onTryHit(target, source, effect) {
				...
				if (!(target.isGrounded()||target.ability==='dragonblessing')) {
			...
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Psychic' && (attacker.isGrounded()||attacker.ability==='dragonblessing') && !attacker.isSemiInvulnerable() && !attacker.hasItem('safarihelmet')) {
	...
	rapidspin: {
		...
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				...
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','tastytreats'];
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
	...
	risingvoltage: {
		...
		basePowerCallback(source, target, move) {
			if (this.field.isTerrain('electricterrain') && (target.isGrounded()||target.ability==='dragonblessing') && !source.hasItem('safarihelmet')||target.hasItem('safarihelmet')) {
	...
	solarbeam: {
		...
		onBasePower(basePower, pokemon, target) {
			const weakWeathers = ['raindance', 'primordialsea', 'sandstorm', 'hail', 'snow','acidrain','pollen'];
	...
	terrainpulse: {
		...
		onModifyType(move, pokemon) {
			if (!(pokemon.isGrounded()||pokemon.ability==='dragonblessing')||pokemon.hasItem('safarihelmet')) return;
		...
		onModifyMove(move, pokemon) {
			if (this.field.terrain && (pokemon.isGrounded()||pokemon.ability==='dragonblessing') &&!pokemon.hasItem('safarihelmet')) {
				move.basePower *= 2;
	...
	tidyup: {
		...
		onHit(pokemon) {
			...
			const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge','tastytreats'];
	...
	hauntedterrain: {
		...
		condition: {
			...
			onDeductPP(target, source) {
				if((source.isGrounded()||source.ability==='dragonblessing')&& !source.hasItem("safarihelmet") && !source.hasType("Ghost"))
				{
				return 1;
				}
			},
	...
	draconicterrain: {
		...
		condition: {
			...
			onModifyMove(move,attacker, defender) {
				if((attacker.isGrounded()||attacker.ability==='dragonblessing') && (defender?.isGrounded()||defender?.ability==='dragonblessing') && !attacker.hasItem('safarihelmet')&& !defender?.hasItem('safarihelmet'))
				{
			...
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Dragon' && (attacker.isGrounded()||attacker.ability==='dragonblessing') && !attacker.isSemiInvulnerable() && !attacker.hasItem('safarihelmet')) {
