...
describe(`Royal Shield`, function () {
	...
	it(`should lower the Atk of a contactor by 2 in Gen 7`, function () {
		battle = common.gen(7).createBattle([
			...
			[{species: "Aegislash", ability: 'stancechange', moves: ['royalshield']}],
		]);
		battle.makeChoices('move zenheadbutt', 'move royalshield');
	...
	it(`should lower the Atk of a contact-move attacker in 2 levels even if immune`, function () {
		...
		battle.makeChoices('move drainpunch', 'move royalshield');
		assert.statStage(battle.p1.active[0], 'atk', -1);
	});
});

describe(`Royal Shield [Gen 6]`, function () {
	afterEach(() => battle.destroy());
